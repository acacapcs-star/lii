...
14 個功能從兩面卡牆改為五個分頁，依三個問題分類：我會主動打開它嗎（工具）、它是我產生的嗎（記錄、收藏）、調一次就不再碰嗎（設定）。

| 分頁 | 路由 | 內容 |
|---|---|---|
| 首頁 | `/home` | 今天的狀態、四個輸入、今日待辦、這個月 |
| 記錄 | `/trends` | 趨勢、本週人設、年度總覽、日記時間軸 |
| Luna | `/voice` | 中央鈕。短按進語音頁，長按直接錄音 |
| 工具 | `/tools` | 思考教練、你常掉進哪一個、希望盒，以及四個當下就能用的方法 |
| 我的 | `/profile` | 我的 Pacers、我的專屬格言、匯出報告、設定 |

底部導覽列在 `lib/core/widgets/lii_bottom_nav.dart`。側欄保留為第二條路徑。
