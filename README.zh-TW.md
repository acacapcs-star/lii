...
**分級**:紅 ≥ 70、黃 ≥ 40、綠 < 40。ERS 引擎與 check-in 的 risk engine 用同一組分界線。
...
AI 回覆語言跟介面語言是分開的設定，因為它們回答的是不同問題。介面語言是你讀的，回覆語言是 Luna 寫的。

| 模式 | 行為 |
|---|---|
| **跟著我打的語言**（預設） | 回覆的語言跟著那則訊息走。打中文回中文、打日文回日文，中英夾雜的訊息也會夾雜著回 |
| **一律用繁體中文** | 不論輸入什麼都用中文回 |
| **一律用英文** | 不論輸入什麼都用英文回 |

預設那個做法是在系統提示後面接一句，請模型用使用者剛剛使用的語言回答——不是在 Dart 裡做字元範圍偵測。手寫的偵測器得把每種文字系統都列出來；交給模型判斷則不需要維護那份清單，日文韓文都涵蓋得到。

只影響兩個地方：**聊聊**與**便條紙的吐槽**。介面文字、按鈕與標題仍然跟著 App 的語言設定。

**這需要你自己的 API 金鑰。** 沒有金鑰時是離線模式，回覆來自固定的本地內容，語言跟著 App 設定。設定面板上會直接說明這件事，而不是讓這個選項看起來壞掉。

onboarding 之後會跳一次說明，因為沒人知道的設定跟不存在的設定是一樣的。

檔案：`core/network/ai_lang_pref.dart`（enum、提示指示、以及儲存的偏好）。
...
14 個功能在首頁分成四區，依三個問題分類：我會主動打開它嗎（工具）、它是我產生的嗎（收藏）、調一次就不再碰嗎（設定，留在側欄）。

| 區 | 卡片 |
|---|---|
| **探索自己** | Check-in · 睡眠紀錄 · 聊聊 · 嘿，在嗎？ |
| **工具** | 我的水晶 · 最近的起伏 · 心情工具箱 · 匯出報告 |
| **看看自己** | 我的專屬格言 · 我的 Pacers |
| **今日待辦** | 心晴筆記（卡片內可捲）· 年度總覽（可展開） |
...
|---|---|---|
| 首頁 | `/home` | 上面四區 |
| 記錄 | `/trends` | 趨勢圖、7/14/30 日滑桿、個人與同齡對照 |
| 工具 | `/tools` | 四個當下就能用的方法，方塊卡排列 |

導覽列在 `lib/core/widgets/lii_bottom_nav.dart`。側欄保留為第二條路徑，含報告、安全流程與設定。

## 完整功能地圖

33 條路由。側欄是 17 個項目的平面清單，底部導覽列放最常用的三個。
...
| Toolbox | 心情工具箱,四個工具:自我對話卡、4-7-8 呼吸、5-4-3-2-1 五感回神、情緒詞彙庫;含練習紀錄 |
...
首頁另有可拖曳的 Luna Pacer 浮球(夜空與玻璃兩面,可左右滑動轉面),以及水晶收藏:6 顆,ice 起始、sea 3 次呼吸、amethyst 7 次、amber 14 次、moss 連續 3 天、dawn 連續 7 天,系統會提示下一顆還差多少。浮球點下去的呼吸方式依 ERS 決定，門檻和 ERS 引擎、首頁狀態文字同一組(0–39 綠、40–69 黃、70 以上紅)。點兩下浮球會叫出 Joy-Con 樣式的控制器,固定在畫面下方中間:左邊搖桿移動位置(推越遠走越快),右邊按住「＋」「－」連續放大縮小,◎ 回到預設位置;大小範圍 44–180,位置與大小都會記住。
...
引擎層                        ers_engine
               ...
               ┌───────────────────┼───────────────────┐
               ▼                   ▼                   ▼
分層層      GREEN 0–39         AMBER 40–69         RED 70–100
            不撤回              撤回追問             撤回排名比較
               │                   │                   │
               ▼                   ▼                   ▼
...
系統用到的方法不多，每一個都是為了某個特定性質而選。比單一選擇更重要的是「哪些被刻意放在一起用」，以及為什麼單獨用一個不夠。

### 五個方法

| 方法 | 用在 | 為什麼選它 |
|---|---|---|
| **分段正規化** | 語速、睡眠時長、負向詞密度 | 兩端都是風險。線性映射會把「太快」和「太慢」放在同一條斜線的兩端，抹掉「中間才是安全」這件事 |
| **加權融合** | ERS：語言 0.40 ＋ 情緒 0.35 ＋ 作息 0.25 | 三串訊號的可靠度不同，影響力就該不同。語言權重最高，因為滑桿可以隨便填，語音比較難刻意操控 |
| **滾動平均** | ERS，三日視窗 | 一天不好不是趨勢 |
| **非對稱遲滯** | 累積風險，12 階量尺 | 升上去和降下來不該一樣容易 |
| **關鍵詞加權比對** | `risk_engine`，當下判讀 | 需要回傳可解釋的理由，不是一個看不懂的數字 |
...
每分鐘 100 字和每分鐘 450 字都是訊號，但它們是**不同的**訊號——一個讀作遲緩，一個讀作激動。線性正規化沒辦法表達這件事。睡眠時長也一樣：超過九小時跟睡太少一樣會被記下來。

這一步在任何加權之前就先做，每個特徵各自處理。這裡算錯，後面全部跟著錯，所以它放在純邏輯裡並且有測試。

### 搭配起來才成立的地方

**滾動平均 ＋ 非對稱遲滯。** 這兩個處理不同的時間尺度，單獨用哪一個都不夠。

三日滾動平均把每日分數抹平，所以一個難過的晚上不會讓分層直接跳上去。但只有平滑會遺忘——三天狀態好就足以抹掉前面三週的困難。
...
升上去只要一天，降下來要三天。**系統刻意不願意太快放掉注意力。** 一個困難了兩週的學生，某個下午心情不錯，並不代表他好了，介面也不該表現得像他好了。

兩者每天最多更新一次，所以重複開啟 App 不會把數字灌上去。

**ERS ＋ risk_engine。** 兩層回答不同的問題，而且從不合併。

| | ERS | risk_engine |
|---|---|---|
| 問題 | 這個人最近的趨勢如何？ | 我眼前這句話說了什麼？ |
| 輸入 | 三串訊號，三天 | 這一則訊息 |
| 輸出 | 0–100 加一個分層 | 一份理由清單 |
| 時間尺度 | 天 | 秒 |

分開是因為合併之後結果就無法解釋了。輔導老師看到「ERS 68」不會知道為什麼；`risk_engine` 回傳的是實際比對到的語句，以及抵銷它們的保護因子。

**保護因子是負權重，不是另外跑一輪。** 近七天完成三次以上工具的人扣 10 分；訊息裡出現求助意圖扣 10 分；睡眠困難趨穩扣 5 分：

```dart
score -= 10;   // 近 7 天完成三次以上工具
score -= 10;   // 訊息中出現求助意圖
score -= 5;    // 睡眠困難趨穩
```

這是一個設計主張，不是技術上的方便：**願意伸手拿工具的人，跟不拿的人處境不同**，即使兩人自評的心情一樣。

**沉默偵測 ＋ 不一致偵測。** 兩個偵測器，對應主訊號失效的兩種方式。

`silence_detector` 處理「完全沒有輸入」的情況——三日警示、七日危急。不寫本身就是資訊。

`incongruence_detector` 處理「有輸入但對不上」的情況。它計四個維度——代名詞密度、認知僵化標記、事件嚴重度關鍵詞、情緒強度——並回報後兩者之間的**落差**。用很平的語氣描述很嚴重的事，是一個特定的模式，而 ERS 和關鍵詞比對都抓不到它。

### 刻意不用的東西

**不對使用者資料做機器學習。** 權重由兩位輔導老師依經驗設定，不是學出來的。35 個自選樣本、沒有對照組，擬合出來的模型會過擬合，而且沒辦法向被擬合的那個人解釋。

**風險不用分類器。** `risk_engine` 回傳的是理由清單而不是機率，因為輔導老師需要知道是什麼觸發了這個判讀，才有辦法不同意它。

**不用語意相似度。** 意義層次的比對會比詞表敏感得多，但那需要把日記內容送到伺服器。隱私設計禁止這件事，所以在這裡，能力較差的本地方法才是正確的選擇。

## Gleam 微光 —— 顏色為什麼來自字典

在微光頁上方選一種心情，或在情緒詞彙庫替一個感覺叫出名字，罐子裡就會留下一顆玻璃球。整頁就是一個玻璃罐，球可以依日期排，也可以讓同色的聚在一起。

練習（呼吸、五感回神）目前**不會**留下球；頁面上的說明文字與篩選都依這個實際行為呈現。

### 顏色不是使用者選的，也不是從 ERS 推的

第一版的設計是「做完練習之後選一個顏色」。寫下來就看得出問題：**使用者不知道該選什麼。**「我剛做完五感回神練習」這件事沒有任何地方指向青色或琥珀色，所以他會點最近的那一個，而那個選擇不帶任何資訊。

第二個選項是依 ERS 分層自動配色——紅燈那天的球是紅的。那個被否決的理由不同：**它在標記記憶。** 一個人回頭看一罐紅球，不是在回顧自己的一個月，是在被展示「你那陣子有多糟」。

顏色改成從情緒詞彙庫來。那個字典本來就有六個大類，而 `GlassTone` 本來就有六色，一對一對得上：
...
**字典不再是唯讀的。** 在這之前，使用者可以打開它、找到一個更精準的詞、然後關掉——什麼都沒留下。現在把一個感覺叫出名字會留下東西。

**沒有多餘的步驟。** 選詞的同時顏色就決定了，使用者從頭到尾不會看到調色盤。

**顏色帶著意義。** 一週下來一整排琥珀色，那就是一週的疲憊，而那一排比任何一張同樣資料做成的圖表都直接。

### 球存什麼、什麼是現算的

球把情緒組存成一個欄位，而不是從顏色反推。上面那張對應表之後可能會改，而一顆記錄為「累」的球，不該因為顏色表被編輯過就安靜地變成別的情緒。
...
// 手動留的——什麼都不存，顯示時才算 group.label(zh)
MemoryBallStore.addFromEmotion(group: g, word: '');
```

理由是**存下來的標籤會被凍結在當時的語言裡**。手動留的球如果存了「難過」三個字，使用者之後切成英文，那顆球還是顯示「難過」——因為那個字串被寫死進資料了。存空的、顯示時現算，語言才跟得上設定。

**從字典挑的詞剛好相反。** 那是使用者真的選的那個詞，用的是他當時在想這件事的語言。事後翻譯過去會失真。

### 連結一張卡

選完詞之後，App 問要不要寫點什麼。選擇要寫的話會開啟 Card Studio——那是既有的格言卡編輯器，有字體、背景和選配的照片——寫完回來自動連上那顆球。

球是在卡片**之後**才建立的，不是之前。反過來的話，只要有人中途放棄編輯，就會留下一顆沒有連結的球。

那個連結只存卡片的 id，不存內容的副本。卡片之後可能被編輯，而一顆球顯示著使用者早就改掉的那句話，比什麼都不顯示更糟。卡片被刪掉時，展開的球會直接說「那張卡已經不在了」，而不是假裝連結還在。

### 在微光頁直接留一顆

罐子上方直接放著六顆情緒球，不用再按「＋」。流程是：

1. **點一個顏色**，例如「生氣」。
2. **選是哪一種**：列出字典裡這一組的詞（惱怒、委屈、被冒犯、不甘心），每個詞附一句說明；說不上來也可以只留大類。詞直接取自情緒詞彙庫（`emotionWordsOf`），不另外維護一份；選的詞也記進和字典同一份紀錄（`recordEmotionWord`）。
3. **寫便條**（選填，最多 300 字），存在球的 `memo` 欄位，和 `note`（那個詞）分開，詞才能繼續拿來分類。
4. **落進罐子**：新的那顆從上方落下、彈一下，周圍泛起兩圈光的漣漪（`DropEcho`）。只有這一顆跑動畫，只用圓形描邊不用模糊濾鏡；系統開「減少動態效果」時略過。

### Luna 的回聲

寫了便條，Luna 會回一兩句話（`gleam_reply.dart`），存在球的 `reply` 欄位，之後點開球還看得到，也可以「換一句」。

- **有設定 AI 金鑰**：每次隨機換一個回應角度（接住感受、看見做到的事、讓感覺變得正常、下一分鐘能做的小事、回應某個細節），並告訴模型上一句是什麼、不要用同樣的開頭。刻意限制在一兩句，不列建議、不診斷、最多問一個問題。
- **沒有金鑰或連線失敗**：用本機的句子，每個情緒組 8 句，隨機挑且不重複上一句。
- **便條出現高風險字眼時**：不送 AI，顯示 App 既有的安全訊息（`aiHighRiskSafetyReply`），關閉後前往 `/safety`。判斷用 `RiskEngine.mentionsHighRisk()`，和 `evaluateDay` 共用同一份關鍵詞清單。

### 隱私：便條會不會離開手機

本 App 承諾日記留在手機裡，而產生 Luna 的回應需要把便條送到 AI 服務。所以輸入框下方會依情況直接說明：

| 情況 | 顯示 |
|---|---|
| 沒設定 AI 金鑰 | 只存在這支手機，不會上傳 |
| 有金鑰，「讓 Luna 回應」開啟（預設） | 會送到 AI 服務，讓 Luna 回應你 |
| 有金鑰，開關關閉 | 只存在這支手機，Luna 不會回應 |

開關關閉時若便條有高風險字眼，仍會顯示本機的安全訊息——那段不經過 AI。

### 清空與保存多久

- **清空**：右上角可以清空整個罐子，先確認一次，清空後 6 秒內可以復原。
- **這些光要留多久**：使用者自己選「一直留著」（預設）、「留一學期」（126 天）或「留一個月」。預設不自動刪除，因為球是使用者自己留下的，App 不該替他決定哪一段該忘。換成較短的時間時，會先說明有幾顆會被收走，確認後才執行。
- 罐子最多 500 顆。到 450 顆時頁面會先提醒「罐子快滿了」，超過上限後最舊的才會被收走，不會默默消失。

### 同樣六種玻璃色，三個地方三種規則

| 地方 | 顏色怎麼決定 | 理由 |
|---|---|---|
| Pacer Lift（語錄纜車） | 使用者自由選 | 那是他收藏的一句話，外觀由他決定 |
| Luna Pacer 呼吸頁 | 只能選已解鎖的水晶；選了會記住，首頁浮球也換成同一色 | 顏色是練習換來的，不是設定裡挑的；換來的要看得到 |
| 微光 | 由情緒組決定，不能選 | 顏色要帶著意義，一整排琥珀色就是一週的疲憊 |
