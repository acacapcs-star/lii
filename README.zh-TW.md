...
首頁另有可拖曳的 Luna Pacer 浮球(夜空與玻璃兩面,可左右滑動轉面),以及水晶收藏:6 顆,ice 起始、sea 3 次呼吸、amethyst 7 次、amber 14 次、moss 連續 3 天、dawn 連續 7 天,系統會提示下一顆還差多少。點兩下浮球會叫出 Joy-Con 樣式的控制器,固定在畫面下方中間:左邊搖桿移動位置(推越遠走越快),右邊按住「＋」「－」連續放大縮小,◎ 回到預設位置;大小範圍 44–180,位置與大小都會記住。
...
- **沒有金鑰或連線失敗**：用本機的句子，每個情緒組 3 句，隨機挑且不重複上一句。
...
- 目前另有 500 顆的總上限，超過時最舊的會被移除；之後預計改為快滿時先提醒。
...
| Luna Pacer 呼吸頁 | 只能選已解鎖的水晶 | 顏色是練習換來的，不是設定裡挑的 |
